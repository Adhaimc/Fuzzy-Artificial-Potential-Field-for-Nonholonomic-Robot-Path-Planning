function fig_benchmark_2()
% FIG_BENCHMARK_2: Generate publication figure for Benchmark II (Nonholonomic)
% Shows trajectory comparison: Fuzzy-APF (Left) vs Classical Khatib (Right)
% Display: 20 trajectories overlaid, 100 obstacles, differential-drive constraints

  fprintf('=== GENERATING FIGURE: Benchmark II (Nonholonomic) ===\n');
  
  % Run benchmark simulations to get trajectory data
  res = benchmark_2_nonholonomic();
  ...
  % ===== LEFT PANEL: Fuzzy-APF Trajectories (Nonholonomic) =====
  ax1 = subplot(1, 2, 1);
  hold on; axis equal;
  ...
  cmap = hsv(numel(traj_fuzzy));
  for i = 1:numel(traj_fuzzy)
    traj = traj_fuzzy{i};
    if (~isempty(traj) && size(traj, 1) > 1)
      % Plot trajectory with rainbow color
  ...
  % ===== RIGHT PANEL: Khatib APF Trajectories (Nonholonomic) =====
  ax2 = subplot(1, 2, 2);
  hold on; axis equal;
  ...
  % Save figure
  drawnow;
  print(gcf, 'comparison_fuzzy_vs_khatib_nonholonomic.png', '-dpng', '-r150');
  print(gcf, 'comparison_fuzzy_vs_khatib_nonholonomic.pdf', '-dpdf');
  fprintf('✓ Saved: /Users/adhaimc/Documents/GitHub/Fuzzy_APF26A/comparison_fuzzy_vs_khatib_nonholonomic.png\n');
  fprintf('✓ Saved: /Users/adhaimc/Documents/GitHub/Fuzzy_APF26A/comparison_fuzzy_vs_khatib_nonholonomic.pdf\n');
